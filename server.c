...
Date: June 10, 2018
Description: The code for the server
*/
...
/* Function: is_connected(int sock)
Description: to check if the client is connected or not */
...
/* Structure: cltinf
Description: Client socket structure, contains client socknmbr,ip and username */
struct cltinf {
	int socknmbr;
...
/* variables */ 
int clients[100]; // Client status , socknmbr stored in the arrays
...
/* Function: sendtoClient(char *msg, int curr)
Description:Send message only to a particular client */
...
/* Function: sendtoallClient(char *msg,int curr,char *username)
Description: Messages sent by the client, received by the server and sent to all clients */
...
/* Function: *rcvmsg(void *sock)
Description: Start/create receive message thread when a client sends a request to connect to the server
STATEFUL : implements the DFA of the protocol */
void *rcvmsg(void *sock)
{
	...
	while((len = recv(cl.socknmbr,msg,500,0)) > 0) {
		...
		j=0;
		int u;
		//Take the message out from the message string received
...
{
	/* variables */
	struct sockaddr_in my_addr, their_addr; //sockaddr structure is used to define a socket address which is used in the bind(), connect()
	int my_sock; //store the client socket
	int their_sock; //server socket from which message is received
	int their_addr_size;
	int portno; //Port number of server on which client is connected
	pthread_t sendt,recvt; //pthread sender and receiver thread
	char msg[500];
	int len;
	struct cltinf cl; //Store client info in the client structure
	char ip[INET_ADDRSTRLEN];
	...
	while(1) {
		// CONCURRENT: Now wait for a new connection
		...
		//Mutex lock, so that till it is disconnected
		pthread_mutex_lock(&mutex);

		//Get socket /client info wanted to connect
		inet_ntop(AF_INET, (struct sockaddr *)&their_addr, ip, INET_ADDRSTRLEN);
		printf("%s :Wants to connect to the server\n",ip);
		cl.socknmbr = their_sock;

		//Send a message to client to send hello to start the authentication
